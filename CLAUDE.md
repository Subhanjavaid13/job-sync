...
- Key-gated (skip themselves until env is set): adzuna (`ADZUNA_APP_ID/KEY`), jsearch (`RAPIDAPI_KEY`). The cron is **twice daily** (03:00 + 13:00 UTC = 08:00 + 18:00 PKT) — owner decision (2026-08-26) so same-day postings are caught; each run emails at most one digest and only when there is something new. ~60 runs/month is still within every free tier, so no per-source rate gating. JSearch uses the v5 API's `/search-v2` endpoint (`/search` no longer exists).
